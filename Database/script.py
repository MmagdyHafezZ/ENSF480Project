...
def process_airports(input_file, output_file):
    ...
    with open(output_file, 'w') as file:
        for airport_code, airport_info in data.items():
            if airport_info['iata'] and airport_info['city'] and airport_info['state'] and airport_info['country'] and (len(airport_info['iata']) == 3):  # Check if IATA code is not empty
                iata = airport_info['iata'].replace("'", "\\'")
